...
<table cellpadding="3" cellspacing="0" class="MyTable">
  ...
  <% While ((Repeat1__numRows <> 0) AND (NOT Recordset1.EOF)) %>
    <tr>
      <td  background="graphics/reliefbg.gif"><div align="right"><a href="Orders.asp?Supplier=<%=(Recordset1.Fields.Item("DD_SUPPLIER_CODE").Value)%>&amp;Customer=<%=(Recordset1.Fields.Item("ACCOUNT_REF_NO").Value)%>&amp;TPName=<%=request.QueryString("TPName")%>&FLA=<%=(Recordset1.Fields.Item("DELIVERY_ADDR1").Value)%>&SLA=<%=(Recordset1.Fields.Item("DELIVERY_ADDR2").Value)%>" class="style5"><%=(Recordset1.Fields.Item("ACCOUNT_REF_NO").Value)%></a></div></td>
