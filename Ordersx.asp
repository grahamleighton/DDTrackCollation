...
PageSQLOriginal = "SELECT  PRIMARY_ITEM_IND,NO_OF_LABELS,cast(QUANTITY as float) as QTY,DATE_OF_ORDER, COMP_NAME, DD_SUPPLIER_CODE,ACCOUNT_REF_NO,  DELIVERY_ADDR1,  DELIVERY_ADDR2,  DELIVERY_ADDR3 "
PageSQLOriginal = PageSQLOriginal & ", DELIVERY_ADDR4,  DELIVERY_ADDR5,  DELIVERY_ADDR6,  DELIVERY_PCODE,  DD_BARCODE_NO,  INVOICE_NO,  CATALOGUE_NUMBER,  orders.OPTION_NUMBER,  orders.ITEM_DESCRIPTION "
PageSQLOriginal = PageSQLOriginal & ",  CAST(WEIGHT AS FLOAT) AS WT,  CAST(PRICE AS FLOAT) AS PC,  DD_PARENT_NO,DD_BARCODE_ORIG, orders.OPTION_DESCRIPTION,CAST(CPC as FLOAT) as C_PC,CAST(CWT as FLOAT) as C_WT "
PageSQLOriginal = PageSQLOriginal & ",ORDER_KEY,LABEL_LEGEND  FROM ORDERS "
PageSQLOriginal = PageSQLOriginal & "inner join stock_items on orders.dd_supplier_code = stock_items.supp_code and orders.catalogue_number = stock_items.cat_no and orders.option_number = stock_items.opt_no "
PageSQLOriginal = PageSQLOriginal & " where coalesce(stock_items.never_collate,0) <> 1 and   tp91_Tstamp is not null and tp93_tstamp is null and TP94_TSTAMP is null "
PageSQLOriginal = PageSQLOriginal & " and tp9a_tstamp is null and tp9b_tstamp is null and tp9k_tstamp is null and tp9d_tstamp is null and (out_of_stock=0 or out_of_stock is null ) "
PageSQLOriginal = PageSQLOriginal & " and dd_supplier_code='<<SUPPLIERCODE>>' and account_ref_no='<<ACCOUNTREF>>' and DELIVERY_ADDR1='<<DELADDR1>>' and DELIVERY_ADDR2='<<DELADDR2>>' "
PageSQLOriginal = PageSQLOriginal & " order by <<SORTFIELD>> <<SORTORDER>> "


Dim PageSQL
PageSQL = replace(PageSQLOriginal , "<<SUPPLIERCODE>>" , replace(Recordset1__MMColParam,"'","''") )
PageSQL = replace(PageSQL , "<<ACCOUNTREF>>" , replace(Recordset1__MMColParam2,"'","''" ) )
PageSQL = replace(PageSQL , "<<DELADDR1>>" , replace(Session("FLA"),"'","''" ) )
PageSQL = replace(PageSQL , "<<DELADDR2>>" , replace(Session("SLA"),"'","''" ) )
PageSQL = replace(PageSQL , "<<SORTFIELD>>" , replace(Session("SortBy"),"'","''" ) )
PageSQL = replace(PageSQL , "<<SORTORDER>>" , replace(Session("SortOrder"),"'","''" ) )
