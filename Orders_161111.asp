    ...
    <tr id="tr_id_<%=dynaID%>"> 
      ...
      <td>
		...
		if len(trim(Recordset1.Fields.Item("DD_BARCODE_NO").Value)) = 16  and Recordset1.Fields.Item("WT").Value < 15 and Recordset1.Fields.Item("PC").Value < 350 then
 				...
 				%>
			  			<a href="#" onclick="AddData('<%=trim((Recordset1.Fields.Item("DD_Barcode_NO").Value))%>','<%=(Recordset1.Fields.Item("WT").Value)%>','<%=(Recordset1.Fields.Item("PC").Value)%>','<%=(Recordset1.Fields.Item("INVOICE_NO").Value)%>','<%=(Recordset1.Fields.Item("ACCOUNT_REF_NO").Value)%>','tr_id_<%=dynaID%>')">Collate</a>
			<% else %>
