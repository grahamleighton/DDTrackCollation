...
<%
' making the lowecase letter to uppercase...
%>
<script type="text/javascript">
...
if request.form("srchSubmit")="Search" then
	...
	Session("txtS") = txtSearch		
	if  txtSearch = "" then
		Response.Redirect("ItemScreen.asp?next=ItemScreen&Supplier="&request.QueryString("Supplier")&"&TPName="&request.QueryString("TPName"))
	end if
...
if txtSearch <> "" or Session("txtS") <> "" then 
	if len(trim(txtSearch))<> 0 then
			SearchString=" AND (uCase(SUPP_PRODUCT_CODE) LIKE '%" & txtSearch & "%' OR ITEM_DESCRIPTION LIKE '%" & txtSearch & "%' OR OPTION_DESCRIPTION LIKE '%" & txtSearch & "%')" 
	else

		if len(trim(Session("txtS")))<> 0 then
				SearchString=" AND (uCase(SUPP_PRODUCT_CODE) LIKE '%" & Session("txtS") & "%' OR ITEM_DESCRIPTION LIKE '%" & Session("txtS") & "%' OR OPTION_DESCRIPTION LIKE '%" & Session("txtS") & "%')" 
		end if		
	end if		
end if
%>
	...
	For Each variableNames in Request.Form
		If Instr(variableNames, "selected") = "1" Then
			...
			If Not rsProd2.EOF Then
				...
				'if the spp_product_code is not null
		...
		End if 
			...
			if isNull (Request("txtSRUpdate")) Then 
				...
				strSQLCmd = "UPDATE DB2ADMIN.STOCK_ITEMS SET SUPP_PRODUCT_CODE=NULL WHERE ID=x'" & prodCode4 & "'"
...
URLStr1 = MM_keepMove
on error resume next
CurrentOffset1 = CInt(request.QueryString("Offset"))
if err.number <> 0 then
    CurrentOffset1 = 0
end if
on error goto 0

URLVar = split(URLStr1 , "&")
URL_a = 0
URLStr1 = "?"
while ( URL_a <= UBound(URLVar))
	'response.write(CStr(URL_i) + "&nbsp;" + URLVars(URL_i) + "<BR>")
	if left(UCase(URLVar(URL_a)),6) <> "OFFSET" and left(URLVar(URL_a),4) <> "RTAG" and left(ucase(URLVar(URL_a)),9) <> "AMP;TXTSE" and left(ucase(URLVar(URL_a)),5) <> "TXTSE" then
    	if URLStr1 <> "?" then
        	URLStr1 = URLStr1 & "&" & URLVar(URL_a)
        else
            URLStr1 = URLStr1 & URLVar(URL_a)
        end if
   end if
...
MM_urlStr = Request.ServerVariables("URL") & URLStr1 & MM_moveParam & "="



MM_moveFirst = MM_urlStr & "0"
'move first ..... search functionality
if txtSearch <> "" Then
...
'move last..... search functionality
if txtSearch <> "" Then
...
'move prev... search functionality...
if txtSearch <> "" Then
...
<table width="1006" height="32" border="0" align="center">
  <tr>
  <td width="534"><p align="left"><span class="style2">Enter Keyword</span><span class="style32">: </span>
    <label>
    <input name="txtSearch" type="text" class="style2" id="txtSearch" onkeyup="caps(this)" value="<%=Session("txtS")%>"/>
  </label>
    <input name="srchSubmit" type="submit" id="srchSubmit" value="Search"  />
  </p></td>
    <td width="462"><span class="style30"><span class="style33">&nbsp;&nbsp;</span></span><span class="style2">Supplier Reference Update:</span><span class="style30"><br />
        <input name="txtSRUpdate" type="text" class="style2" id="txtSRUpdate" value="<%=Request.Form("txtSRUpdate")%>"/><br/>
        <input name="Save" type="submit" id="Save" value="Save" />
        <button type="button" value="clear" id="clear">Clear</button>
    </span></td>
  </tr>
</table>
